...
async def search_route(request: Request):
    ...
    try:
        ...
        searched_bookmarks: List[Bookmark] = search_bookmarks(
            query, page=page, per_page=25
        )
        all_search_results: List[Bookmark] = search_bookmarks_all(
            query
        )  # For total count
        logger.info(
            f"🔍 Search completed successfully, found {len(searched_bookmarks)} bookmarks on page {page}, total: {len(all_search_results)}"
        )
        ...
        logger.info(
            f"🔍 Creating components with {len(searched_bookmarks)} search results"
        )
        return Div(
            NavMenu(bookmark_count=len(all_search_results)),
            SearchBar(query=query),
            BookmarkImageList(
                bookmarks=searched_bookmarks
            ),  # Changed to BookmarkImageList for screenshots by default
        )
...
async def get_thumbnail_route(request: Request) -> HTMLResponse:
    ...
    if bookmark and bookmark.get("thumbnail_url"):
        ...
        img_component = PreviewImage(
            src=bookmark["thumbnail_url"], id=f"thumbnail-{bookmark_id}"
        )
    ...
    img_component = PreviewImage(
        src=None, id=f"thumbnail-{bookmark_id}"
    )  # Shows placeholder
    img_html = to_xml(img_component)
...
async def check_if_bookmark_already_saved_route(request: Request):
    ...
    if bookmark:
        ...
        return Response(
            content=json.dumps(
                {"status": "not_found", "message": "Bookmark does not exist."}
            ),
            status_code=404,
            media_type="application/json",
        )
...
async def add_bookmark_route(request: Request):
    ...
    if existing_bookmark:
        if force_update:
            ...
            return Response(
                "Bookmark already exists. Not updated unless forceUpdate is true.",
                status_code=409,
            )
    ...
    headers = {"HX-Trigger": json.dumps({"showToast": f"New bookmark added: {title}"})}
    from starlette.responses import Response
...
async def update_route():
    try:
        ...
        return JSONResponse(
            {
                "status": "success",
                "message": "Database backed up and RSS feed updated successfully.",
            }
        )
    except Exception as e:
    ...
    try:
        ...
        if "/random" in referer:
            ...
            if bookmarks:  # Make sure we have bookmarks left
                ...
                # No bookmarks left
                return HTMLResponse(
                    "No more bookmarks available to choose from.", status_code=200
                )
...
async def update_bookmark_route(bookmark_id: str, request: Request):
    ...
    logging.info(
        f"UPDATE_BOOKMARK_ROUTE: Parsed data - title: {title}, url: {url}, description: {description}, tags: {tags}"
    )

    # Basic validation (can be expanded)
    if not title or not url:
        logging.error(
            f"UPDATE_BOOKMARK_ROUTE: Validation failed - title: {title}, url: {url}"
        )
    ...
    try:
        ...
        logging.info(
            f"UPDATE_BOOKMARK_ROUTE: Starting database updates for bookmark {bookmark_id_int}"
        )
        ...
        logging.info(
            f"UPDATE_BOOKMARK_ROUTE: Database updates completed for bookmark {bookmark_id_int}"
        )

        updated_bookmark = await fetch_bookmark_by_id(id=bookmark_id)
        logging.info(
            f"UPDATE_BOOKMARK_ROUTE: Fetched updated bookmark: {updated_bookmark is not None}"
        )

        if not updated_bookmark:
            logging.warning(
                f"UPDATE_BOOKMARK_ROUTE: Updated bookmark not found for id {bookmark_id}"
            )
            return HTMLResponse(
                status_code=200,
                headers={"HX-Reswap": "none", "HX-Trigger": "closeModal"},
            )
        ...
        logging.info(
            f"UPDATE_BOOKMARK_ROUTE: Setting HX-Trigger header for bookmark id: {bookmark_id}"
        )
        ...
        logging.info(
            "UPDATE_BOOKMARK_ROUTE: Response created using to_xml(), returning to client"
        )
        return response
